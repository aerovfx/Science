# Bài 27: Thuật toán tìm kiếm và sắp xếp

> **Câu hỏi lớn:** Hai cách giải đúng có thể khác nhau về số bước như thế nào?  
> **Sản phẩm:** Phòng thí nghiệm thuật toán trên mảng nhỏ  
...
1. Giải thích các ý chính: linear search; bubble sort; trace; số phép so sánh.
...
micro:bit hoặc MakeCode Simulator, thẻ số. Chuẩn bị thêm giấy A4, bút, thước, nhật ký kỹ thuật và vật liệu chế tạo nếu cần.
...
**Câu hỏi suy nghĩ:** So sánh số bước khi tìm phần tử đầu, giữa, cuối và không tồn tại.

## 4. Kiến thức cốt lõi — 15 phút

- Từ khóa: linear search; bubble sort; trace; số phép so sánh.
...
```text
for each item compare target; for passes compare adjacent and swap if left>right
```
...
6. **Lưu phiên bản:** đặt tên tệp `Lop_Nhom_Bai27_v1` và chụp sơ đồ kết nối.

### Tiêu chí thành công

- [ ] trace đúng từng bước và đếm số phép so sánh.
...
- Phản tư cá nhân 3–5 câu và tên phiên bản cuối `Lop_Nhom_Bai27_final`.
